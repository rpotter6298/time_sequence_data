...
class cyt_time_sequence_module:
    """
    A class for processing and analyzing time sequence data from a data_class object.
    """

    def __init__(self, data_module, variable: str, time_limits: dict = None):
        """
    ...
    def normalize(self, data, variable, robust=True):
        ...
        def norm(data, variable):
            ...
            data["Normalized_Measurement"] = data.apply(
                lambda row: row[variable] / initial_values[row["Specific_Identifier"]],
                axis=1,
            )
            return data
    ...
    def calc_change_rate(self, data):
        """
        Calculates the growth rate by taking the difference in Normalized Measurement between consecutive time points.

        :param data: DataFrame containing the normalized data.
        :return: DataFrame with growth rate data.
        """
        growth_rate = data.copy()
        growth_rate["Rate_of_Change"] = growth_rate.groupby(
            ["Treatment", "Specific_Identifier"]
        )["Normalized_Measurement"].diff()
        data["Rate_of_Change"] = growth_rate["Rate_of_Change"]
        return data
